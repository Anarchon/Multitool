...
bool Supervisor::send_command(const std::string& instance_id, const std::string& payload) {
  auto it = instances_.find(instance_id);
  if (it == instances_.end()) return false;
  auto& instance = it->second;
  if (instance.status != InstanceStatus::Running) return false;

  // Production: send `cmd=command;payload=...` through instance-specific IPC channel.
  std::cout << "[supervisor] cmd -> " << instance_id << " payload=" << payload << '\n';
  return true;
}


bool Supervisor::query_tab_view(const std::string& instance_id, std::string& out_json) {
  auto it = instances_.find(instance_id);
  if (it == instances_.end()) return false;
  auto& instance = it->second;
  if (instance.status != InstanceStatus::Running) return false;

  // Production: send `cmd=command;payload={action="render_tab"}` over instance IPC
  // and parse plugin response JSON into UI model.
  if (instance.plugin_id == "firefox.gecko") {
    out_json =
        R"({"toolbar":[{"type":"address","bind":"url"}],"body":{"type":"html","mime":"text/html","content":"<div id=\"root\"><h3>Gecko HTML UI</h3><p>Plugin liefert eigenes HTML pro Instanz.</p></div>","styles":["body{font-family:sans-serif;}"],"scripts":["window.__PLUGIN_READY__=true;"]}})";
  } else if (instance.plugin_id == "chromium.cef") {
    out_json = R"({"toolbar":[{"type":"address","bind":"url"}],"body":{"type":"webview","engine":"cef"}})";
  } else {
    out_json = R"({"toolbar":[],"body":{"type":"panel","text":"Plugin content"}})";
  }

  std::cout << "[supervisor] render-tab -> " << instance_id << '\n';
  return true;
}
