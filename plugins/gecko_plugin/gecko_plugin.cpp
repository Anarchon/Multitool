...
bool command(const char* command_json, char* response_buf, std::uint32_t response_buf_size) noexcept {
  ...
  if (cmd.find("render_tab_html") != std::string::npos) {
    const std::string html =
        "<!doctype html><html><body style='font-family:sans-serif'>"
        "<h3>Firefox Gecko Plugin</h3><p>Current URL: " + g_current_url +
        "</p><p>This HTML is rendered by the plugin instance itself.</p></body></html>";
    const std::string json =
        std::string("{\"body\":{\"type\":\"html\",\"mime\":\"text/html\",\"content\":\"") +
        html +
        "\"},\"styles\":[\"body{font-family:sans-serif;background:#111;color:#eee;}\"],"
        "\"scripts\":[\"window.geckoPluginBoot=()=>console.log('gecko boot');geckoPluginBoot();\"]}";
    write_buf(response_buf, response_buf_size, json);
    return true;
  }

  if (cmd.find("render_tab") != std::string::npos) {
    const std::string json =
        std::string("{\"toolbar\":[{\"type\":\"address\",\"bind\":\"url\",\"value\":\"") +
        g_current_url +
        "\"}],\"body\":{\"type\":\"webview\",\"engine\":\"gecko\"}}";
    write_buf(response_buf, response_buf_size, json);
    return true;
  }
