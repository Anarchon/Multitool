...
Zusätzlich hat jeder Gecko-Tab in der Main-GUI eine **Adressleiste**:
- Eingabe einer URL im Tab
- Main sendet `navigate` an genau die zugehörige `instance_id`
- Andere Tabs/Instanzen bleiben unbeeinflusst

Damit lassen sich mehrere Firefox/Gecko-Instanzen parallel in getrennten Host-Prozessen testen.


## 14) Plugin-eigenes Tab-Rendering

Die Main-GUI besitzt nur den **Tab-Container** (Lebenszyklus, Close-Button, Status).
Die eigentlichen Inhalte kommen plugin-seitig:
- Main fragt pro Instanz `render_tab` (über Supervisor/IPC) ab
- Plugin liefert ein UI-JSON-Modell (Toolbar + Body)
- Main rendert dieses Modell im zugehörigen Tab

Damit kann jedes Plugin sein Tab-Layout selbst bestimmen, ohne andere Tabs zu beeinflussen.


Hinweis Gecko-HTML:
- Gecko kann über `render_tab_html` ein HTML-basiertes Tab-UI liefern (inkl. Toolbar/Body-Daten).
- Main rendert dieses HTML nur im zugehörigen Instanz-Tab; andere Instanzen bleiben isoliert.

- Für HTML-Tabs kann das Plugin zusätzlich CSS/JS-Blöcke liefern (`styles`, `scripts`).
